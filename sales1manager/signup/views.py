...
def email(request):
    ...
    user_id = request.POST['signupID']
    user_pw = request.POST['signupPW']

    # 이름/이메일의 길이 초과시
    if len(user_id) >= 200:
        message = '입력하신 아이디가 너무 길어요.'
    ...
    #PW 암호화
        ...
        new_user = User(user_id = user_id, user_email = user_email , user_name = user_name, user_pw = encrypted_userPW, user_validation = "General")
        new_user.save()
        code = randint(1000, 9999)
        content = {'verifyCode':code}
        msg_html = render_to_string('signup/email_format.html',content)
        msg = EmailMessage(subject="인증코드 발송 메일",body=msg_html,from_email="mhappy78@gmail.com",bcc=[user_email])
